...
const Game = () => {
    ...
    // Catch Pokemon
    // Get the username and id from pokemonData and currentUser in local storage
    // Send a post request to the pokedex database to add the caught Pokemon to the caughtpokemon table
    
    // set the username and id to state
    // send a post request to the pokedex database to add the caught Pokemon to the caughtpokemon table
    ...
    const catchPokemon = async () => {
        ...
        const id = pokemon.id;
        try {
            await axios.post(`http://localhost:3001/users/${username}/pokemon/${id}`);
        } catch (error) {
